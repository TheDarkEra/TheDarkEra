...
public class TeleporterDark extends Teleporter {
	...
	public boolean placeInExistingPortal(Entity par1Entity, double par2,
			...
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						if (worldServerInstance.getBlock(k1, i2, l1) == TDEBlocks.dark_portal) {
							while (worldServerInstance.getBlock(k1, i2 - 1, l1) == TDEBlocks.dark_portal) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				destinationCoordinateCache.add(j1, new PortalPosition(i, j, k,
			...
			if (worldServerInstance.getBlock(i - 1, j, k) == TDEBlocks.dark_portal) {
				j2 = 2;
			}
			...
			if (worldServerInstance.getBlock(i, j, k - 1) == TDEBlocks.dark_portal) {
				j2 = 3;
			}
			...
			if (j2 > -1) {
				...
				boolean flag1 = !worldServerInstance.isAirBlock(i + i3 + k3, j,
						k + j3 + l3)
						|| !worldServerInstance.isAirBlock(i + i3 + k3, j + 1,
								k + j3 + l3);
				boolean flag2 = !worldServerInstance.isAirBlock(i + i3, j, k
						+ j3)
						|| !worldServerInstance.isAirBlock(i + i3, j + 1, k
								+ j3);
				...
				if (flag1 && flag2) {
					...
					int i4 = k - l3;
					d4 -= (double) l3;
					flag1 = !worldServerInstance.isAirBlock(k1 + i3 + k3, j, i4
							+ j3 + l3)
							|| !worldServerInstance.isAirBlock(k1 + i3 + k3,
									j + 1, i4 + j3 + l3);
					flag2 = !worldServerInstance
							.isAirBlock(k1 + i3, j, i4 + j3)
							|| !worldServerInstance.isAirBlock(k1 + i3, j + 1,
									i4 + j3);
				}
			...
			for (j2 = k - b0; j2 <= k + b0; ++j2) {
				...
				for (k2 = worldServerInstance.getActualHeight() - 1; k2 >= 0; --k2) {
					if (worldServerInstance.isAirBlock(i2, k2, j2)) {
						while (k2 > 0
								&& worldServerInstance.isAirBlock(i2, k2 - 1,
										j2)) {
							--k2;
						}
			...
			for (i2 = i - b0; i2 <= i + b0; ++i2) {
				...
				for (j2 = k - b0; j2 <= k + b0; ++j2) {
					...
					for (k2 = worldServerInstance.getActualHeight() - 1; k2 >= 0; --k2) {
						if (worldServerInstance.isAirBlock(i2, k2, j2)) {
							while (k2 > 0
									&& worldServerInstance.isAirBlock(i2,
											k2 - 1, j2)) {
								--k2;
							}
		...
		for (k2 = 0; k2 < 4; ++k2) {
			...
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					j3 = j5 + l2;
					i4 = j2 + (i3 - 1) * l5;
					worldServerInstance.notifyBlocksOfNeighborChange(k3, j3,
							i4, worldServerInstance.getBlock(k3, j3, i4));
				}
			}
		}
