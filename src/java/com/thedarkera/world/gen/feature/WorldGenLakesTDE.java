package com.thedarkera.world.gen.feature;
...
public class WorldGenLakesTDE extends WorldGenerator {

    private Block field_150556_a;
    private static final String __OBFID = "CL_00000418";

    public WorldGenLakesTDE(Block block)
    {
        ...
        else
        {
            ...
            for (i1 = 0; i1 < 16; ++i1)
            {
                for (j2 = 0; j2 < 16; ++j2)
                {
                    for (j1 = 4; j1 < 8; ++j1)
                    {
                        if (aboolean[(i1 * 16 + j2) * 8 + j1] && world.getBlock(x + i1, y + j1 - 1, z + j2) == TDEBlocks.dark_dirt && world.getSavedLightValue(EnumSkyBlock.Sky, x + i1, y + j1, z + j2) > 0)
                        {
                            BiomeGenBase biomegenbase = world.getBiomeGenForCoords(x + i1, z + j2);

                            if (biomegenbase.topBlock == Blocks.mycelium)// change to dark mycelium if thats a thing
                            {
                                world.setBlock(x + i1, y + j1 - 1, z + j2, Blocks.mycelium, 0, 2);
                            }
                            else
                            {
                                world.setBlock(x + i1, y + j1 - 1, z + j2, TDEBlocks.dark_grass, 0, 2);
                            }
                        }
                    }
                }
                ...
                for (i1 = 0; i1 < 16; ++i1)
                {
                    for (j2 = 0; j2 < 16; ++j2)
                    {
                        for (j1 = 0; j1 < 8; ++j1)
                        {
                            ...
                            {
                                world.setBlock(x + i1, y + j1, z + j2, TDEBlocks.dark_stone, 0, 2);
                            }
                        }
                    }
                }
            ...
            if (this.field_150556_a.getMaterial() == Material.water)
            {
                for (i1 = 0; i1 < 16; ++i1)
                {
                    for (j2 = 0; j2 < 16; ++j2)
                    {
                        ...
                        {
                            world.setBlock(x + i1, y + b0, z + j2, Blocks.ice, 0, 2);//change to dark ice if thats a thing
                        }
                    }
                }
            }
