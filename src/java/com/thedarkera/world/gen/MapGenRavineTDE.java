...
public class MapGenRavineTDE extends MapGenBase {
    ...
    private boolean isTopBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == TDEBlocks.dark_grass : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        Block top    = (isExceptionBiome(biome) ? TDEBlocks.dark_grass : biome.topBlock);
        Block filler = (isExceptionBiome(biome) ? TDEBlocks.dark_dirt  : biome.fillerBlock);
        Block block  = data[index];

        if (block == TDEBlocks.dark_stone || block == filler || block == top)
        {
            if (y < 10)
            {
